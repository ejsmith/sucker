...
Review also caught repeated avatar requests. The [before report](avatars-before.png) grows from nine to eleven lookups while paging the same opponents; the [after report](avatars-after.png) stays at one through paging and refresh. Avatar results, including missing photos, are cached per signed-in owner for five minutes. Lookup requests are capped at 50 IDs per sequential batch, and the current user's avatar continues to use the live profile value. A 110-profile unit case checks batch sizes, deduplication, and empty input; the full browser fixture checks reuse with real local profile requests.
