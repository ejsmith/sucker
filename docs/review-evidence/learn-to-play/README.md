...
The guide does not alter gameplay or force an introduction before playing. Its token step follows the current counterpunch rules.
...
After integrating the current app, the token step still described Mulligan as computer-only. `integration-before.png` reproduces that stale instruction. `integration-after.png` was refreshed after the September 15 rebase and shows the current Mulligan and counterpunch explanation at 375 × 667; scroll the guide to read the remaining text on this smaller viewport. `counterpunch-after.png` shows the entire token explanation at 430 × 932. Four guide cases and the two-player browser flow pass after the rebase, including 393 × 852 interaction checks, and both typechecks pass. The games-list baseline also includes the current Jab behavior.
