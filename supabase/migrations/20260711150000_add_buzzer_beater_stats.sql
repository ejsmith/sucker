...
add column buzzer_beater_win integer not null default 0;

alter table public.head_to_head_stats
add column buzzer_beater_wins integer not null default 0;
...
with effective_turns as (
  select game_id, player_id, category, score, turn_index
  from public.turns
  where status in ('submitted', 'blocked', 'finalized')
),
last_winner_turns as (
  select result.game_id, result.player_id, result.opponent_id, max(turn.turn_index) as final_turn_index
  from public.game_player_results result
  join effective_turns turn
    on turn.game_id = result.game_id
   and turn.player_id = result.player_id
  where result.won
  group by result.game_id, result.player_id, result.opponent_id
),
scores_before_final_turn as (
  select
    winner.game_id,
    winner.player_id,
    winner.opponent_id,
    coalesce(sum(turn.score) filter (where turn.player_id = winner.player_id), 0) +
      case when coalesce(sum(turn.score) filter (
        where turn.player_id = winner.player_id
          and turn.category in ('ones', 'twos', 'threes', 'fours', 'fives', 'sixes')
      ), 0) >= 63 then 35 else 0 end as player_score,
    coalesce(sum(turn.score) filter (where turn.player_id = winner.opponent_id), 0) +
      case when coalesce(sum(turn.score) filter (
        where turn.player_id = winner.opponent_id
          and turn.category in ('ones', 'twos', 'threes', 'fours', 'fives', 'sixes')
      ), 0) >= 63 then 35 else 0 end as opponent_score
  from last_winner_turns winner
  left join effective_turns turn
    on turn.game_id = winner.game_id
   and turn.turn_index < winner.final_turn_index
  group by winner.game_id, winner.player_id, winner.opponent_id
),
buzzer_beater_results as (
  select score.game_id, score.player_id
  from scores_before_final_turn score
  join public.game_player_results result
    on result.game_id = score.game_id
   and result.player_id = score.player_id
  where score.player_score <= score.opponent_score
    and result.final_score > score.opponent_score
)
update public.game_player_results result
set buzzer_beater_win = 1
where exists (
  select 1
  from buzzer_beater_results buzzer
  where buzzer.game_id = result.game_id
    and buzzer.player_id = result.player_id
);

update public.head_to_head_stats stats
set buzzer_beater_wins = coalesce((
  select sum(result.buzzer_beater_win)::integer
  from public.game_player_results result
...
begin
  ...
  insert into public.computer_stats (
    ...
    sucker_hunts, sucker_hunt_misses, sucker_punches_used, sucker_blockers_used,
    sucker_tokens_spent, average_sucker_tokens_spent, sucker_tokens_leftover,
    average_sucker_tokens_leftover, computer_highest_score, computer_total_score,
    computer_average_score, computer_upper_bonus_games, computer_sucker_games,
    computer_three_of_a_kind_games, computer_four_of_a_kind_games, computer_full_house_games,
    computer_small_straight_games, computer_large_straight_games
  )
  values (
    ...
    greatest(sucker_punches_used, 0), greatest(sucker_blockers_used, 0),
    greatest(sucker_tokens_spent, 0), greatest(sucker_tokens_spent, 0),
    greatest(sucker_tokens_leftover, 0), greatest(sucker_tokens_leftover, 0),
    greatest(computer_score, 0), greatest(computer_score, 0), greatest(computer_score, 0),
