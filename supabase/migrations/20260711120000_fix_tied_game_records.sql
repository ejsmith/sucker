with tied_games as (
  ...
  group by game_id
  having count(*) = 2
    and min(final_score) = max(final_score)
)
update public.games
set winner_id = null
where id in (select game_id from tied_games);
...
update public.game_player_results
set
  won = false,
  blowout_win = 0,
  blowout_loss = 0,
  comeback_win = 0
where game_id in (select game_id from tied_games);

delete from public.head_to_head_stats;

with result_outcomes as (
  select
    result.*,
    min(final_score) over (partition by game_id) = max(final_score) over (partition by game_id) as tied
...
select
  ...
  sum(case when not won and not tied then 1 else 0 end)::integer as losses,
